...
**질문 자체가 안 읽히면** 아래의 **무슨 뜻이죠?** 를 누르세요. 같은 질문이 한국어로 펼쳐집니다
(쉬운 말로 바꾸는 게 아니라 그대로 옮긴 것입니다). 기본은 접혀 있습니다 — 영어를 먼저 읽어볼
순서를 뺏지 않으려고요. 질문을 만들 때 같이 받아두는 거라 **요청은 안 씁니다.**
...
여력이 없는 날은 홈에서 **가볍게 한 문장**으로 시작해도 그날은 연습한 날로 그대로 셉니다.
...
홈 화면 아래쪽에 **최근 30일 중 며칠 연습했는지**, 총 대화 수, 누적 단어 수, 배운 표현 수, 도달 레벨,
...
트로피는 30일 중 7일, 하루 200단어, 표현 30개 같은 지점에서 열립니다.

**"연속 며칠"이 아닌 이유** — 원래는 하루 유예 2회짜리 연속 사슬이었는데, 실제 데이터에서
**연습한 날에 숫자가 내려갔습니다** (8/18에 나왔는데 15일 → 11일). 유예 창이 미끄러지면서
과거 사슬을 소급해서 잘랐기 때문입니다. 굴러가는 집계는 끊길 것도, 소급해서 잘릴 것도 없고
연습한 날엔 절대 안 내려갑니다. [기획서 5.7절](docs/product-spec.md)에 데이터가 있습니다.
...
맥에 기본으로 깔리는 영어 음성은 compact 방식이라 로봇 소리가 납니다. 쓸 만하게 하려면
**시스템 설정 → 손쉬운 사용 → 말하기 콘텐츠 → 시스템 음성 → 음성 관리…** 에서 해당 억양의
**Enhanced/Premium**을 내려받고, **시스템 음성으로 지정**하세요.

앱은 이 순서로 고릅니다:

1. `NEXT_PUBLIC_SPEECH_VOICE`에 이름을 적었으면 그것
2. **브라우저가 `default`로 표시한 음성** — 시스템 설정에서 지정한 그 음성입니다.
   단, 가르치는 억양과 맞을 때만 (미국 시스템 음성이 호주 영어를 덮지 않게)
3. 최신 세대 음성 이름 (`Catherine` · `Gordon` · `Allison` …)
4. 그래도 없으면 억양만 맞는 아무거나

**2번이 핵심입니다.** 이름으로 "(Premium)"을 찾는 방법은 **안 통합니다** — 크롬은 이름을
현지화하면서 꼬리표를 떼서, 업그레이드된 Karen과 기본 Karen이 **이름도 `voiceURI`도 똑같이**
`Karen (영어(오스트레일리아))`로 옵니다. 둘을 가르는 유일한 신호가 `default` 플래그입니다.
...
거절당한 횟수만큼 대시보드보다 적게 나옵니다. `TTS=system`이면 이 막대는 아예 안 그립니다 —
안 쓰는 예산을 남았다고 할 이유가 없으니까요.

- 라우트가 실패하면 조용히 기기 음성으로 내려가고, **그때만** 쉐도잉 카드에 그렇게 적힙니다.
  `TTS=system`으로 일부러 기기 음성을 쓰는 중이면 아무 말도 하지 않습니다 — 정상 동작을
  고장으로 읽히게 하면 안 되니까요.
...
src/
  ...
  lib/
    ...
    stats.ts                    순수 계산 — 연습 리듬, 실수 추이, 트로피 판정
